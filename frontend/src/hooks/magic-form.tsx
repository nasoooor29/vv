...
type FieldConfig<T extends ZodObject<any> = ZodObject<any>> = {
    ...
    label?: string
    placeholder?: string
    type?: 'text' | 'email' | 'password' | 'textarea' | 'date' | 'select' | 'multi-select' | 'number' | 'slider'
    options?: string[]
    description?: string
    min?: number
    max?: number
...
export function useFormGenerator<T extends ZodObject<any>>(
    ...
    const cleanDataBeforeSubmit = (data: any): T['_output'] => {
        ...
        Object.keys(cleaned).forEach((key) => {
            ...
            if (isFieldOptional(key) && fieldSchema.def?.type === 'date' && cleaned[key] === '') {
                cleaned[key] = undefined
            }

            // Handle empty number strings for optional number fields
            if (isFieldOptional(key) && fieldSchema.def?.type === 'number' && cleaned[key] === '') {
                cleaned[key] = undefined
            }
        })
        return cleaned
    ...
    const getFieldType = (fieldName: string): string => {
        ...
        if (typeName === 'string') {
            ...
            if (fieldName.toLowerCase().includes('password') || fieldName.toLowerCase().includes('pwd')) {
                return 'password'
            }
            return 'text'
        }
            ...
            case 'select':
                return (
                    <Select value={field.value || ''} onValueChange={field.onChange}>
                        <SelectTrigger>
                            <SelectValue placeholder={fieldConfig.placeholder || `Select ${fieldName}...`} />
                        </SelectTrigger>
                        <SelectContent>
    ...
    >(({ name, label, placeholder, type, options, description, min, max, step, ...props }, ref) => {
        const nameStr = String(name)
        const defaultType = getFieldType(nameStr) as 'text' | 'email' | 'password' | 'textarea' | 'date' | 'select' | 'multi-select' | 'number' | 'slider'
        const finalType = (type || defaultType) as 'text' | 'email' | 'password' | 'textarea' | 'date' | 'select' | 'multi-select' | 'number' | 'slider'
            ...
            <FormField
                ...
                render={({ field }) => (
                    <FormItem ref={ref} {...props}>
                        ...
                        <FormControl>
                            {renderFieldInput(nameStr, { name, label: finalLabel, placeholder, type: finalType, options, min, max, step }, field)}
                        </FormControl>
    ...
    const FullFormComponent = () => {
        return (
            <FormComponentImpl className="space-y-4">
                {formConfig.formTitle && <h2 className="text-lg font-semibold">{formConfig.formTitle}</h2>}
                {formConfig.formDescription && <p className="text-sm text-muted-foreground">{formConfig.formDescription}</p>}
                <ErrorsComponent />
    ...
    const fragmentedForm = {
        ...
        formTitle: formConfig.formTitle ? <h2 className="text-lg font-semibold">{formConfig.formTitle}</h2> : null,
        formDescription: formConfig.formDescription ? <p className="text-sm text-muted-foreground">{formConfig.formDescription}</p> : null,
        wrapper: (props?: React.HTMLAttributes<HTMLFormElement> & {
        ...
        field: (fieldProps: FieldConfig<T> & React.HTMLAttributes<HTMLDivElement>) => (
